...
import { ConnectionModuleConfiguration } from '../connection-module.configuration';
...
export abstract class SocketService {
  ...
  protected constructor(
    private readonly configuration: ConnectionModuleConfiguration,
  ...
  initialize(): Observable<void> {
    ...
    const socket = io([this.configuration.url, this.namespace].join('/'), {
      autoConnect: false,
  ...
  protected propagateStateChange(state: NetworkState): void {
    this.store.dispatch(connectionActions.changeState({ namespace: this.namespace, state }));
  }
}
